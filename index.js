...
function getLastCarInfo(arr) {
  let carMake = arr[arr.length-1].car_make
  let carModel = arr[arr.length-1].car_model
  return `This is a ${carMake} ${carModel}`
}
...
function getOlderCars(arr, maxYear) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].car_year <= maxYear) {
      maxYearArr.push(arr[i])
    }
  }
  return maxYearArr

  // answer using .filter:
  // return arr.filter(year => year.car_year <= maxYear)
}
